...
async def solve(task_data: TaskData):
    ...
    work_end = 20   # End of workday in "hours" after 8 AM
    # Constants
    ...
    for task_id, task in enumerate(tasks_data):
        job_start, job_end = job_time_windows.get(task_id, (0, 20))
        ...
        for subtask_id, (skill_required, duration, required_personnel) in enumerate(task):
            ...
            # AddMaxEquality =========== > target == Max(variables)
    ...
    for technician, tasks in technician_tasks.items():
        for i in range(len(tasks)):
            for j in range(i + 1, len(tasks)):
                task1 = tasks[i]
                task2 = tasks[j]
                # AddNoOverlap : A NoOverlap constraint ensures that all present intervals do not overlap in time.
